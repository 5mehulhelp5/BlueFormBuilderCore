<?php
    ...
    class EmailNotification extends DataObject
    {
        ...
        if (!$form || !$form->getId()) {
            ...
            if ($submission && method_exists($submission, 'getFormId')) {
                $formId = (int)$submission->getFormId();
                if ($formId > 0 && isset($this->formFactory)) {
                    try {
                        $loadedForm = $this->formFactory->create()->load($formId);
                        if ($loadedForm && $loadedForm->getId()) { $this->setForm($loadedForm); $form = $loadedForm; }
                    } catch (\Throwable $e) { if (isset($this->logger)) { $this->logger->error('EmailNotification::sendAdminNotification: autoload form failed', ['form_id'=>$formId,'error'=>$e->getMessage()]); } }
                }
            }
        }
        if (!$form || !$form->getId()) { if (isset($this->logger)) { $this->logger->error('EmailNotification::sendAdminNotification: no form bound'); } return false; }
$form       = $this->getForm();
        $submission = $this->getSubmission();
